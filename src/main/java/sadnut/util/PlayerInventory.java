...
public class PlayerInventory
{
	...
	private static ResourceLocation invTexture = new ResourceLocation(Reference.MOD_ID, Reference.AssetPathGui + "gui_inventory_lower.png");
	...
	private static void addToolbarSlots(Container container, InventoryPlayer inventory, int x, int y, boolean freezeSlot) 
	{
		for(int j = 0; j < 9; ++j)
		{
			if(freezeSlot && j == inventory.currentItem)
				addSlotToContainer(container, new Slot(inventory, j, x + j * 18, y+58));//changed from SlotForShowOnly to Slot
			else
	...
	{
		CoreUtils.bindTexture(invTexture);
